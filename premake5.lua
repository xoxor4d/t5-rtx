...
workspace "t5-rtx"
	...
	project "start-sp"
		...
		if(os.getenv("CODBO1_ROOT")) then
			print ("Setup paths for sp using environment variable 'CODBO1_ROOT'")
			targetdir(os.getenv("CODBO1_ROOT"))
			debugdir (os.getenv("CODBO1_ROOT"))
			debugcommand (os.getenv("CODBO1_ROOT") .. "/t5sp.exe")
		end
	...
	project "t5-rtx"
		...
		postbuildcommands {
			"echo ----------------------------",
			"echo Rename \"t5-rtx.dll\" to \"t5rtx.asi\"",
			"MOVE /Y \"$(TargetDir)t5-rtx.dll\" \"$(TargetDir)t5rtx.asi\"",
		}
